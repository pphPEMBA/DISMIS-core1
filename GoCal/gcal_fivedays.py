...
#gcal5days = '/home/d-slave1/d1_SuperDismis/DISMIS-core/SystemService/DisArcade/gcal5days.txt'
...
""" IMPORING PROFILE """
from Core.profile import Ctoken_pickle,Ccredentials,gcal5days,slave_sender,slave_passwd,receiver\
    ,temporaryfiles, accept_path, gcal5days
'--------------------------------------------------------------------------------------------------------------------------------------'
'--------------------------------------------------------------------------------------------------------------------------------------'

from pyfiglet import Figlet
def banner(gcal5days):
    custom_fig = Figlet(font='graffiti')
    poster = custom_fig.renderText('Dismis')
    #print(custom_fig.renderText('Dismis'))
    d=open(gcal5days,'a+')
    d.write("\n" + poster)
def extractTime(gcal5days):
    import datetime
...
def mailer(slave_sender, slave_passwd, receiver):
    try:
        ...
        msg['Subject'] = "GOOGLE CALENDAR 5DAYS!"# storing the subject  
        body = ''    # string to store the body of the mail
        msg.attach(MIMEText(body, 'plain'))     # attach the body with the msg instance 
        filename = "gcal5days.txt"    # open the file to be sent  
        attachment = open(gcal5days, "rb") 
        ...
        print(' ')
        Log_Time()
        print('Google Calendar 5 days mail sent')
        print(' ')
        print(' ')
        print('\t\t\t\tSkill: gcal_fivedays')
        print('--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
        speak('Google Calendar 5 days mail sent boss.')
    except socket.gaierror:
        pass        

def fivedays(accept_path):
    os.system('aplay ' + accept_path +' &')
    #try:
    banner(gcal5days)
    extractTime(gcal5days)
    firstCal()
    secondCal()
    thirdCal()
    forthCal()
