...
#gcal20days = '/home/d-slave1/d1_SuperDismis/DISMIS-core/SystemService/DisArcade/gcal20days.txt'
...
""" IMPORING PROFILE """
from Core.profile import Ctoken_pickle,Ccredentials,gcal20days,slave_sender,slave_passwd,receiver\
    ,temporaryfiles, accept_path
'--------------------------------------------------------------------------------------------------------------------------------------'
'--------------------------------------------------------------------------------------------------------------------------------------'

from pyfiglet import Figlet
def banner(gcal20days):
    custom_fig = Figlet(font='graffiti')
    poster = custom_fig.renderText('Dismis')
    #print(custom_fig.renderText('Dismis-HA'))
    d=open(gcal20days,'a+')
    d.write("\n" + poster)
def extractTime(gcal20days):
    import datetime
...
def mailer(slave_sender, slave_passwd, receiver):
    try:
        ...
        msg['Subject'] = "GOOGLE CALENDAR 20DAYS!"# storing the subject  
        body = ''    # string to store the body of the mail
        msg.attach(MIMEText(body, 'plain'))     # attach the body with the msg instance 
        filename = "gcal20days.txt"    # open the file to be sent  
        attachment = open(gcal20days, "rb") 
        ...
        print(' ')
        Log_Time()
        print('Google Calendar 20 days mail sent')
        print(' ')
        print(' ')
        print('\t\t\t\tSkill: gcal_twentydays')
        print('--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
        speak('Google Calendar 20 days mail sent boss.')
    except socket.gaierror:
        pass        

def twentydays(accept_path):
    os.system('aplay ' + accept_path +' &')
    #try:
    banner(gcal20days)
    extractTime(gcal20days)
    firstCal()
    secondCal()
    thirdCal()
    forthCal()
