...
#gcal10days = '/home/d-slave1/d1_SuperDismis/DISMIS-core/SystemService/DisArcade/gcal10days.txt'
...
""" IMPORING PROFILE """
from Core.profile import Ctoken_pickle,Ccredentials,slave_sender,slave_passwd,receiver\
    ,temporaryfiles, accept_path, gcal10days
'--------------------------------------------------------------------------------------------------------------------------------------'
'--------------------------------------------------------------------------------------------------------------------------------------'

from pyfiglet import Figlet
def banner(gcal10days):
    custom_fig = Figlet(font='graffiti')
    poster = custom_fig.renderText('Dismis')
    #print(custom_fig.renderText('Dismis'))
    d=open(gcal10days,'a+')
    d.write("\n" + poster)
def extractTime(gcal10days):
    import datetime
...
def mailer(slave_sender, slave_passwd, receiver):
    try:
        ...
        msg['Subject'] = "GOOGLE CALENDAR 10DAYS!"# storing the subject  
        body = ''    # string to store the body of the mail
        msg.attach(MIMEText(body, 'plain'))     # attach the body with the msg instance 
        filename = "gcal10days.txt"    # open the file to be sent  
        attachment = open(gcal10days, "rb") 
...
def tendays(accept_path):
    os.system('aplay ' + accept_path +' &')
    #try:
    banner(gcal10days)
    extractTime(gcal10days)
    firstCal()
    secondCal()
    thirdCal()
    forthCal()
